...
library SafeMath {
     ...
     *
     * Requirements:pragma solidity 0.6.12;
...
contract GGingPresale is Ownable {
    using SafeMath for uint256;
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "GGingPresale: sale is not open");
        require(!hasEnded(), "GGingyPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "GGingyPresale: min contribution criteria not met"
        );
        require(
    ...
    function claimTokens() external {
        require(hasEnded(), "GGingyPresale: presale is not over");
        require(softCapReached(), "GGingyPresale: soft cap not reached, refund is available");
        require(
            contributions[msg.sender] > 0,
            "GGingyPresale: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "GGingyPresale: presale is not over");
        require(!softCapReached(), "GGingyPresale: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "GGingyPresale: nothing to claim"
        );
    ...
    function endPresale() external onlyOwner {
        require(!finalized, "GGingyPresale: presale is already over");
        finalized = true;
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "GGingyPresale: presale is not over");
        // uint256 unsoldTokens = _getTokenAmount(hardCap.sub(weiRaised));
